...
$column = array('id', 'username', 'type', 'cash', 'diamond', 'site', 'created_at');
...
$username = Anti_xss($_POST['username']);
    if(!@$user){
        ...
        foreach($db->fetch_assoc("SELECT * FROM `users` WHERE `id` != '0' $sql_domain $sql_type $sql_username $orderby LIMIT $start, $length", 0) as $info){
            $detail = json_decode($info['detail'], true);
            $json = array();
            $json[] = $info['id'];
            $json[] = $info['username'];
            $json[] = detail_users($info['username'])['name'];
            $json[] = type_user($info['type']);
            $json[] = number_format($info['cash']);
            $json[] = number_format($info['diamond']);
            $json[] = $info['site'];
            $json[] = '<button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#info" data-cash="'.$info['cash'].'" data-diamond="'.$info['diamond'].'" data-username="'.$info['username'].'" data-type="'.$info['type'].'" data-status="'.$detail['status'].'"><i class="fa fa-eye"></i></button>';
            $json[] = date('H:i d-m-Y',$info['created_at']);
            $data[] = $json;
        }
        $output = array(
            "draw" => intval($_POST["draw"]),
            "recordsTotal" => $db->num_rows("SELECT * FROM `users`"),
            "recordsFiltered" => $db->num_rows("SELECT * FROM `users` WHERE `id` != '0' $sql_domain $sql_type $sql_username"),
            "data" =>  $data
        );
